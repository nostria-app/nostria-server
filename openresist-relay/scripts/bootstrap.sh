...
if [[ "$CURRENT_STRFRY_REF" != "$EXPECTED_STRFRY_VERSION" ]]; then
    echo "WARNING: expected strfry $EXPECTED_STRFRY_VERSION for the relay build" >&2
fi
echo "Building and starting openresist relay stack..."

cd "$PROJECT_DIR"
$COMPOSE_CMD up -d --build openresist-relay
